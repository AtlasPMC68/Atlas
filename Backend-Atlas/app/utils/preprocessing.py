    ...
    *,
    ...
    if enable_white_balance:
        if white_balance_method == "gray_world":
            rgb_lin = white_balance_gray_world(rgb_lin, mask=mask)
        else:
            rgb_lin = white_balance_percentile_white_patch(rgb_lin, percentile=wb_percentile, mask=mask)

    # 3.5) CLAHE (local contrast on L)
    rgb_lin = clahe_l_channel_lab(rgb_lin, clip_limit=0.02, kernel_size=(8, 8))

    # 4) Robust global normalization
    if enable_percentile_norm:
        rgb_lin = percentile_normalize(rgb_lin, p_low=norm_p_low, p_high=norm_p_high, mask=mask)
    
    # 5) Background removal / foreground mask refinement (for non-alpha images)
    if enable_background_mask and bg_method == "paper":
        mask = build_paper_background_mask(rgb_lin, mask, paper_threshold_deltaE=paper_threshold_deltaE)
    ...
    if enable_denoise:
        if denoise_method == "bilateral":
            ...
            rgb_lin = denoise_nl_means_rgb(rgb_lin)
