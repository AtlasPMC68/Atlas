...
def assert_same_mask_pixels(generated_path: str, reference_path: str, color_name: str) -> None:
    """
    Compare two masks by pixels, not by raw bytes.
    """
    gen = read_mask_as_binary(generated_path)
    ref = read_mask_as_binary(reference_path)

    diff = np.mean(gen != ref) * 100.0

    if diff > MAX_MASK_DIFF_PERCENT:
...
def find_reference_by_prefix(directory: str, prefix: str) -> Tuple[str, list[str]]:
    """
...
def run_color_extraction_test(
    ...
    for color_name, spec in expected_colors.items():
        ...
        ref_file, matches = find_reference_by_prefix(etalons_dir, ref_prefix)
        if not ref_file:
            issues.append(
        ...
        try:
            assert_same_mask_pixels(generated_file, ref_file, color_name)
