  ...
  <div class="space-y-4">
    ...
    >
      ...
      <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center">
        <svg v-if="step.status === 'completed'" class="w-5 h-5 text-success" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
        </svg>
        <div v-else-if="step.status === 'processing'" class="loading loading-spinner loading-sm"></div>
...
const stepProgress = computed(() => {
  ...
  const stepProgressPercent = (props.progress - baseProgress) / (100 / steps.length)
